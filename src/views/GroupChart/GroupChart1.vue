<template>
  <div>
    <Layout class="layout zm">
      <!-- 左侧群，联系列表信息 -->
      <Sider hide-trigger class="zm-left-side"
        >Sider
        <!-- 个人聊天列表 -->
      ...
      <Layout class="zm-center">
        ...
        <Content class="zm-right-content">
          ...
          <ul class="zm-msg">
            ...
            >
              <img :src="item.img" class="zm-msg-log" />
            ...
            <!-- 提示有用户进来了 -->
            <div class="system" v-if="systemNews">
              <div>{{ chartDeta }}</div>
          ...
          <Footer class="zm-footer">
            <div>
              <Input
                ...
                class="zm-input"
              />
              <Icon size="35" type="ios-send" @click="postMsg" />
            </div>
          </Footer>
        </Content>
        <!-- 输入框 -->
...
export default {
  ...
  data() {
    return {
      ...
      chartName: "" //服务端返回的进入聊天室的用户名
    };
  },
    watch:{
    intoName(nval,oval){
      console.log(nval)
         this.postChartList(this.userName,nval)
    }
  },
  methods: {
    postChartList(a,b) {
      this.axios
        .post("charts/postChartList", {
          userName: a,
          intoName: b
        })
        .then(res => {
